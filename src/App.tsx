...
const DataView = () => {
  ...
  useEffect(() => {
    ...
    // Newly uploaded files can take a moment to finish processing.
    let attempts = 0;
    const maxAttempts = 5;
  ...
  const isLikelyKrutiDev = (text: string) => {
  ...
  const normalizeText = (value: unknown) => {
    ...
    if (textEncoding === "krutidev" || textEncoding === "krishna") {
      // Best-effort: many Krishna legacy datasets use a similar non-Unicode pattern.
      return krutiDevToUnicode(text);
    }

    // Auto mode: only convert when string strongly looks KrutiDev encoded.
  ...
  const filteredRows = normalizedRows.filter((row) => {
    ...
    if (selectedColumn && selectedTags.length > 0) {
      ...
      if (!selectedTags.includes(cellValue)) return false;
    }
          ...
          <div className="bg-white border border-zinc-200 rounded-2xl p-4 md:p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
              ...
              >
                ...
                <option value="krutidev">Text: KrutiDev-010</option>
...
const DataEntry = () => {
  ...
  const [entryFont, setEntryFont] = useState<"default" | "kruti" | "kokila">(
    "default",
  );
